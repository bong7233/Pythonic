...
전역에 하나만 깔 수 있다면 이렇게 된다.

```text nolines
   system python
        │
        ├── numpy 2.2       <- 프로젝트 B가 요구
        └── numpy 1.26      <- 프로젝트 A가 요구
                               둘 중 하나만 살아남는다. 다른 쪽은 깨진다.
```

프로젝트마다 격리하면 공존한다.

```text nolines
   system python              <- 아무도 건드리지 않는다
        │
   ┌────┴─────┐
   │          │
 project A  project B         <- 각자의 .venv
 numpy 1.26 numpy 2.2            서로를 모른다
```
