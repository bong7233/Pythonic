...
**Part IX를 마쳤다.** NumPy로 배열을 다루고, pandas·polars로 표 형태 데이터를 처리하고, matplotlib으로 눈으로 확인하는 능력까지 갖췄다. 이 책의 마지막 파트에서는 이 모든 걸 실제로 움직이는 로봇에 연결한다.

**다음 절**: [10.1 ROS 2 개요와 설치](#/ros-intro) — 지금까지 배운 파이썬 실력을 로봇 소프트웨어의 표준에 연결할 차례다.
