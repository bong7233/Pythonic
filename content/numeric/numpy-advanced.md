...
## 종합: 왜 이게 수치 계산 전체의 기반인가

이 절에서 본 세 가지 — 스트라이드, ufunc, einsum — 는 전부 같은 질문에 대한 답이다. **"파이썬 반복문을 어떻게 없애는가."** 카메라 이미지나 LiDAR 포인트 클라우드처럼 대량의 수치 데이터를 다루는 코드는 결국 이 세 가지 위에서 돌아간다. 여기서 "뷰가 왜 공짜인지", "ufunc이 왜 빠른지", "축약 순서가 왜 중요한지"를 체감해 두면, 실제 센서 데이터를 실시간으로 처리해야 하는 [10.12 센서 데이터 처리](#/sensors)에서 그 지식을 다시 설명받을 필요가 없다 — 이름만 바뀌어서 나타날 뿐이다.
