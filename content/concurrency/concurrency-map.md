...
이 로드맵을 따라가면, 나중에 [7.15 최단 경로](#/shortest-path)의 다익스트라를 병렬화하거나, [11.13 분산 학습](#/distributed)에서 여러 GPU에 작업을 나누거나, [13.3 rclpy 노드](#/rclpy-node)에서 ROS 2의 콜백이 왜 스레드나 코루틴으로 설계됐는지 이해할 준비가 끝난다.
