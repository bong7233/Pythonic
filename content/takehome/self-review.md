...
플래그 인자를 알아보는 기준은 하나다. **함수 본문의 첫 줄이 그 인자로 갈라지면 그건 두 개의 함수다.** `process` 는 `if cancel:` 로 시작해서 예약 경로와 취소 경로가 끝까지 만나지 않는다. 공유하는 것은 좌석 유효성 검사 한 줄뿐이고, 그건 함수를 따로 부르면 된다.
...
| 주석 처리된 코드 덩어리 | 눈으로. 린터는 못 잡는다 | git이 있는데 왜 남겼는지 설명이 안 된다 |
...
[1.1](#/objects-names)에서 "변수는 상자가 아니라 이름표"라는 한 문장으로 시작했다. 그 이름표는 5부에서 메모리 측정의 근거가 됐고, 10부 끝에서는 다른 언어의 `shared_ptr` 로 다시 나타났다. 방금 이 절에서는 **"이 필드를 누가 읽는가"** 라는 질문으로 한 번 더 나왔다 — 이름과 그 이름이 가리키는 것을 분리해서 볼 줄 아는 능력이, 결국 자기 코드를 남의 눈으로 읽는 능력이었다.
