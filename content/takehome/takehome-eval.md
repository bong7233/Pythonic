...
과제는 이렇다. **이 절 내내 쓸 예제다.** 뒤의 절들은 각각 다른 소재로 같은 판단을 반복한다 — [12.2](#/requirements-to-model)는 사내 비품 재고, [12.4](#/boundaries-di)는 전기차 충전 스탠드다. 소재를 바꾸는 이유는 하나다. **판단이 소재에 딸린 것이 아니라는 것을 보이기 위해서다.**
...
| 언제 쓰나 | 기본. 과제 제출에는 이쪽 | 이미 fixture 때문에 `conftest.py` 가 있을 때. 설정 파일을 안 늘리고 싶을 때도 쓴다 |

**과제형이면 기본은 안 1이다.** 이유는 동작이 아니라 **평가자가 파일 하나를 열어 실행 방법을 알 수 있다**는 데 있다. 빈 파일은 "왜 여기 있는지"를 설명해 주지 않는다. `pyproject.toml` 자체는 [6.5 패키징](#/packaging)에 있다.

다만 안 2도 흔하고, 틀린 것도 아니다. 빈 `conftest.py` 를 쓸 거면 **파일 안에 한 줄 주석으로 왜 있는지 남겨라** — `# tests/ 에서 프로젝트 루트를 import 할 수 있게 한다. 지우면 pytest 가 깨진다.` 한 줄이면 된다. **빈 파일이 문제지 `conftest.py` 가 문제인 게 아니다.** 두 안의 진짜 차이는 `sys.path` 가 아니라 **의도가 기록됐는가**이고, 그건 이 절 전체의 주제이기도 하다.
...
def _nearest(current: int, above: set[int], below: set[int]) -> int:
    ...
    if not below:
        return min(above)
    up, down = min(above), max(below)
    return down if (current - down) < (up - current) else up
```

::: note `_nearest` 의 분기 순서가 저렇게 생긴 이유
비어 있을 수 있는 쪽을 먼저 `None` 에 담아 두고 나중에 `if up is None: return down` 으로 거르는 것이 더 자연스러워 보인다. 그런데 그렇게 쓰면 `down` 의 타입이 `int | None` 이라 `-> int` 와 안 맞고, mypy·pyright가 둘 다 `Incompatible return value type` 을 낸다. 런타임은 멀쩡하다 — 호출부가 `above` 와 `below` 중 최소 하나는 비지 않음을 보장하기 때문이다. **문제는 그 보장이 시그니처에 안 적혀 있다는 것이다.** 위처럼 빈 쪽을 먼저 걸러 내면 타입 검사기도 같은 결론에 도달한다. 바로 아래 상자가 "린터는 당연히 돌려라"라고 말하는데, 그 린터에 걸리는 코드를 모범으로 내놓을 수는 없다.
:::
...
느린 쪽 숫자는 **층 이동 횟수가 정한다.** 이 8개가 유발하는 이동은 총 6회다 — `test_maintenance_mode_returns_to_the_ground_floor` 에서 7층부터 1층까지 내려가는 것이 전부고, 나머지 일곱 개는 층을 한 번도 안 옮긴다. `step()` 이 한 층마다 `time.sleep(0.3)` 하는 가짜 드라이버를 부르도록 바꿔 측정했다. **0.3 × 6 = 1.8 초가 그대로 보고 시간에 얹힌다.** 당신 숫자가 다르면 먼저 이동 횟수를 세라.

테스트 8개에서 이미 100배 안팎이고, 이 차이는 **장치를 부르는 횟수에 정비례해서 커진다.** 여기서 중요한 건 초가 아니다. **느린 테스트는 안 돌리게 되고, 안 돌리는 테스트는 없는 테스트다.** 요구사항 6("장치 없이 테스트")은 편의 요청이 아니라 설계 요청이다 — 같은 문제를 [12.4](#/boundaries-di)가 전기차 충전 스탠드 과제 위에서 푼다.
:::
...
| --- | --- | --- |
| 줄 수(빈 줄·주석 제외, docstring 포함) | 26 | 26 |
...
줄 수는 26 대 26이다. **똑같다.** 그런데 오른쪽의 `DirectionalStrategy.choose` — 이 과제가 실제로 요구한 유일한 규칙 — 은 아직 `...` 한 줄로 비어 있다. **아무것도 안 한 상태에서 이미 왼쪽 전체와 같은 분량이다.** 그 자리에 `next_stop` 의 내용을 채우면(`_nearest` 도 같이 딸려 온다) 46줄이 된다.

그래도 **문제는 줄이 아니다.** 오른쪽을 읽으려면 클래스 넷의 관계를 머리에 올려야 하고, 그러고 나서야 실제 규칙 한 줄에 도달한다. 그 대가로 얻는 것은 **요구사항에 없는 유연성**이다.
...
**6. 확장 과제.** 1번의 프로젝트를 `git clone . /tmp/check` 로 복제해서 `pytest -q` 를 돌려라. 통과했다면, `elevator/rules.py` 를 `.gitignore` 에 넣고 커밋한 뒤 다시 복제해 봐라. **아무 일도 안 일어난다.** 테스트는 그대로 8개 통과한다.

이유는 이렇다 — **`.gitignore` 는 이미 추적 중인 파일에는 효력이 없다.** 무시 목록은 "추적할지 말지"를 정할 때만 읽히고, 한 번 추적에 들어간 파일은 계속 커밋된다. 사고를 재현하려면 추적을 실제로 끊어야 한다.

```bash
git rm --cached elevator/rules.py    # 파일은 남기고 추적만 뗀다
git commit -m "untrack rules.py"
git clone . /tmp/check3 && cd /tmp/check3 && pytest -q
```

이제 `ModuleNotFoundError` 두 개로 수집이 죽는다. 여기서 두 가지를 적어라.

- **복사(`cp -r`)로는 이 사고가 왜 안 잡히는지** 한 문장.
- `.gitignore` 만 고쳤을 때 아무 일도 안 일어난 것이, 제출 직전 검사에 대해 무엇을 말해 주는지 한 문장. (힌트: 당신이 확인해야 하는 것은 무시 목록이 아니라 **`git ls-files` 의 출력**이다.)
:::

**다음 절**: [12.2 요구사항에서 모델로](#/requirements-to-model) — 요구사항 문장에서 명사와 동사를 뽑아 클래스와 메서드로 바꾸는 절차를, 사내 비품 재고 관리 과제 위에서 단계별로 시연한다. 만들지 **않을** 것을 고르는 기준(YAGNI)이 그 절차의 절반이다.
