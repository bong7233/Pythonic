...
`parking/__init__.py` 가 0줄인 것은 **이 시점에는 그 파일이 정말로 비어 있기 때문**이다. 거기에 공개 API를 선언하는 이야기는 [12.7](#/project-structure)에 있다. 그 열 줄을 넣고 같은 명령을 다시 돌리면 해당 행이 `4 0 0 0 100%` 가 되고 TOTAL 은 `74 26 14 3 58%` 로 바뀐다. **테스트를 한 줄도 안 쓰고 커버리지가 2%p 오른다.** import 문 네 줄을 추가했을 뿐인데.
