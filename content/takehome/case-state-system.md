...
[12.3](#/state-machine)이 적은 것은 "조심하라"가 아니라 **배타 조건**이다. `IntEnum` 은 **기존 정수 API(프로토콜, C 라이브러리, DB 코드)와 섞여야 할 때만** 쓴다 — 선택표의 그 칸에 다른 조건은 없다. 화폐는 그 조건이 아니다. 자판기 바깥에 정수 프로토콜도, DB 코드값도 없고, CLI가 경계에서 `Coin(int(value))` 로 한 번 바꿔 넣을 뿐이다. 그런데도 화폐는 `IntEnum` 으로 만들었다. **12.3의 규칙에 두 번째 자리를 하나 더 붙이는 것**이고, 그러니 근거를 대야 한다.
...
그 두 번째 자리의 기준은 이것이다. **값이 도메인에서 의미를 갖고 산술에 쓰이는가.** 발주서 상태의 `1, 2, 3` 은 아무 의미가 없어서 `auto()` 였다. 화폐의 `100` 은 **금액 그 자체**고, `coin * n` 으로 곱해야 하고, 정렬하면 액면 순서가 나온다. 그래서 `IntEnum` 이 맞다.

조건이 하나 더 붙는다. 12.3의 danger 박스가 경고한 사고 — 서로 다른 도메인의 `IntEnum` 두 개가 `==` 로 참이 되어 인자 순서를 바꾼 버그가 조용히 통과하는 것 — 는 **이 프로그램에서는 일어날 수가 없다.** `IntEnum` 이 `Coin` 하나뿐이라 잘못 비교할 상대가 없기 때문이다. 두 번째 `IntEnum` 을 만드는 순간 이 근거는 사라지고, 그때는 12.3의 원래 규칙으로 돌아가야 한다.
...
| 반환이 실패할 수 있는가 | **있다.** 1000원 넣고 반환하면 동전으로 바꿔 줘야 한다 | 거스름돈 계산 때문에 실패하는 일은 없다(하드웨어 고장은 별개) |
| 금고 잔고 | 반환 계산이 틀리면 금고가 어긋난다 | 반환은 금고를 건드리지 않는다 |
| 거스름돈 재원 | 그냥 금고를 본다 | **금고 + 에스크로를 합쳐야 한다** |
| 코드 | 상태가 하나 적다 | `CoinBox` 가 둘이다 |

**안 B를 골랐다.** 근거는 요구사항 6의 "전액"이다. 안 A에서 1000원 지폐를 넣고 반환 레버를 누르면 자판기는 1000원을 동전으로 바꿔 줘야 하고, 동전이 모자라면 **반환이 실패한다.** 손님 돈을 못 돌려주는 상태는 어떤 이유로도 만들면 안 된다. 안 B에서는 **도메인 규칙상** 반환이 실패하지 않는다 — 거스름돈을 만들 필요가 없고, 넣은 물리적 화폐를 그대로 내보내면 되기 때문이다.

여기서 한 단어를 정확히 써야 한다. **"도메인 규칙상"이지 "절대로"가 아니다.** 호퍼가 걸리면 안 B에서도 동전은 안 나간다. 그래서 반환 레버 코드는 뒤에 나올 판매 코드와 **같은 규칙**을 지킨다 — 되돌릴 수 없는 바깥 호출보다 **앞에 되돌릴 것을 두지 않는다.** 반환은 바깥 호출이 하나뿐이라 규칙이 특히 단순해진다. 하드웨어를 먼저 성공시키고, 그다음에 에스크로를 비운다.

```python title="vending/machine.py (발췌)"
def refund(self) -> dict[Coin, int]:
    """반환 레버. 하드웨어를 먼저 부르고, 성공한 뒤에 에스크로를 비운다."""
    target = self._target(MachineEvent.REFUND)
    coins = {c: n for c, n in self._escrow.snapshot().items() if n > 0}

    self._dispenser.return_coins(coins)  # 실패하면 아래 두 줄은 안 돈다

    self._escrow.take(coins)
    self._state = target
    return coins
```

**에스크로를 먼저 비우고 하드웨어를 부르면 안 된다.** `drain()` 한 줄이면 끝나 보이지만, 그렇게 쓰면 `return_coins` 가 실패했을 때 손님 돈은 이미 사라진 뒤다. 남는 자리는 `balance == 0` 인데 상태는 `ACCEPTING` — `states.py` 에서 `ACCEPTING` 을 "투입 금액 > 0"이라고 적어 뒀으니 **존재할 수 없는 조합**이고, 퀴즈 3이 독자에게 찾으라고 시키는 바로 그 종류의 버그다. 읽는 값(`snapshot`)과 빼는 값(`take`)을 나누면 실패한 반환은 그냥 아무 일도 안 일어난 것이 되고, 손님은 레버를 다시 당길 수 있다.

```python title="tests/test_machine.py (발췌)"
def test_반환이_실패하면_손님_돈이_에스크로에_남는다():
    m = machine(EmptyHopper())
    m.insert(Coin.W1000)
    with pytest.raises(ChangeReturnFailed) as exc:
        m.refund()
    assert exc.value.amount == 1000
    assert m.balance == 1000      # 돈은 아직 손님 것이다
    assert m.state is MachineState.ACCEPTING
```

가짜 하드웨어를 **성공 경로에만 물리면 이 버그는 영원히 안 보인다.** `EmptyHopper` 를 판매(`select`)에만 쓰고 반환(`refund`)에는 안 써 본 것이 실제로 이 순서 실수를 오래 살려 뒀다.
...
`SerialDispenser` 가 `Path` 를 받는 것도 의도적이다. 실제 장치 파일이든 로그 파일이든 **여는 대상만 바뀐다.** 장치 자리에 임시 파일을 주고 `coin 1000`, `coin 1000`, `select A1` 을 흘려 넣으면 그 파일에 이렇게 쌓인다(1100원 거스름돈).
...
같은 판매 1,000회를, 가짜 배출기로 한 번과 `SerialDispenser` 로 로컬 파일에 쓰면서 한 번 돌렸다. 판매 한 번마다 하드웨어 호출이 세 번이다 — `dispense` 한 번, 거스름돈 `{500x2, 100x1}` 을 내보내는 `HOPPER` 줄 두 번. 즉 1,000회는 파일 열고 쓰고 닫기 3,000번이다. 각각 일곱 번씩 돌린 범위다.

| 배출기 | 판매 1,000회 |
| --- | --- |
| `FakeDispenser` | 7.8 ~ 10.4 ms |
| `SerialDispenser` (로컬 파일) | 31 ~ 47 ms |

(Python 3.14.0rc2 / Linux 기준 실측. 절대값은 기기마다 다르지만 자릿수 차이는 어디서나 같다.)

4배 안팎이다(재실행마다 3.4~4.9배로 흔들린다). 그리고 이건 로컬 파일이라 OS 캐시에 얹혀 있다. 진짜 시리얼 포트가 얼마나 걸리는지는 **여기서 측정하지 않았으니 숫자를 말하지 않겠다.**

중요한 건 이 표가 경계를 정당화하지 않는다는 것이다. **속도는 부수 효과고, 진짜 이유는 재현성이다.** 모터 걸림·호퍼 고장·전원 순단을 테스트에서 원하는 순간에 정확히 일으킬 수 있는가 — 그게 요구사항 7이 묻는 것이다. 이 저장소의 테스트 30개가 0.02초에 끝나는 것은 좋은 일이지만, 그것 때문에 경계를 그은 것은 아니다.
:::
...
::: danger 그래도 되돌릴 수 없는 구멍이 하나 남는다 — 그걸 숨기지 마라
`dispense` 는 성공했는데 `return_coins` 가 실패하면 어떻게 되는가. 상품은 이미 나갔다. 되돌릴 방법이 없다.

반환 레버 쪽과 비교해 보면 차이가 분명하다. 레버에서 `return_coins` 가 실패하면 그 앞에 되돌릴 것이 **하나도 없어서** 아무 일도 안 일어난 상태로 끝난다. 여기서는 앞에 **되돌릴 수 없는 일(배출)이 이미 하나 있다.** 순서를 아무리 잘 짜도 하드웨어 호출이 두 번이면 그 사이에 구멍이 생긴다. 남는 구멍의 개수는 **되돌릴 수 없는 바깥 호출의 개수 - 1** 이다.
...
30개다. 무엇을 테스트하고 무엇을 안 하는가의 기준은 [12.6](#/test-strategy)에 있으니, 여기서는 **이 과제의 30개가 어디에 붙었는지**만 본다.
...
| `test_machine.py` 하드웨어 실패 | 3 | 배출 실패, 판매 중 거스름돈 반환 실패, 반환 레버 실패 |
...
여기까지 만든 것을 세어 보면 소스 519줄, 테스트 281줄, 패키지 파일 10개다. 과제형에서 **감점의 절반은 여기서 더 만들어서** 난다. 이 저장소가 의도적으로 만들지 않은 것들과 그 판단 근거다.
...
├── pyproject.toml               pythonpath = ["."] — tests 에서 vending 을 import 하게 한다
├── vending/
│   ├── __init__.py
...
디렉터리는 둘이다. 파일이 열 개로 늘었을 뿐 깊이는 그대로다.

[12.7](#/project-structure)의 규모로는 **C**다. 이 판정이 한 번 걸린다 — 12.7이 규모 B의 예시 도메인으로 든 것이 **주차장, 자판기, 도서 대출**이기 때문이다. 그런데 12.7의 판정표는 지시문의 단어로 고르라고 하고, 여기 요구사항 7에는 "하드웨어 없이 테스트할 수 있어야 한다"가 적혀 있다. 판정표에서 **"하드웨어를 흉내 내어"가 이긴다.** 소재가 자판기라서 B인 것이 아니라, 요구사항 7이 붙는 순간 `ports.py` 와 `fakes.py` 가 생겨서 C가 된다. 12.7의 규모 C 트리(`ports` / 진짜 구현 / `fakes`, 디렉터리 두 개)와 이 저장소가 그대로 겹치는 것을 확인해라. **규모는 도메인이 아니라 경계의 개수가 정한다.**

`pyproject.toml` 에서 테스트를 위해 들어간 것은 세 줄이다.

```toml title="pyproject.toml (발췌)"
[tool.pytest.ini_options]
pythonpath = ["."]
testpaths = ["tests"]
```

`pytest` 를 그냥 부르면 `tests/` 만 `sys.path` 에 들어가고 프로젝트 루트는 안 들어간다 — `from vending.coins import Coin` 이 수집 단계에서 죽는다. [12.1](#/takehome-eval)이 이 문제의 두 해법(`pythonpath` 와 빈 `conftest.py`)을 실측으로 비교한 뒤 **과제 제출의 기본은 `pythonpath`** 라고 못 박았고, [12.2](#/requirements-to-model)·[12.6](#/test-strategy)·[12.7](#/project-structure)의 프로젝트가 전부 그쪽이다. 여기도 같다. 이 과제는 `pyproject.toml` 을 어차피 두므로, 설정을 한 파일에 모으는 쪽이 평가자가 열 파일을 늘리지 않는다. 빈 `conftest.py` 를 고를 자리는 [12.10](#/case-domain-repo)처럼 **설정 파일이 아직 하나도 없을 때**고, 그때도 12.1이 못 박은 조건 하나는 지켜야 한다 — 파일 안에 왜 있는지 한 줄 적어라. 빈 파일이 문제지 `conftest.py` 가 문제인 게 아니다.
...
- **반환 레버가 하드웨어 고장으로 실패하면 손님 돈은 에스크로에 남는다.** 돈이 사라지지는
  않고 레버를 다시 당길 수 있지만, 호퍼가 계속 고장이면 꺼낼 방법이 없다. 이때 상태가
  ACCEPTING 이라 관리자가 점검 모드로 들어가지도 못한다 — 물리 열쇠로 여는 통로가 필요하다.
