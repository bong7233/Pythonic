...
`str` 의 `isX()` 메서드는 **빈 문자열에 대해 전부 `False`** 다. 반면 `all(...)` 은 `True` 다. 빈 입력을 별도로 막지 않을 거라면 `isalnum()` 쪽이 안전하다. 반대로 "빈 문자열은 허용"이 요구사항이면 `all` 이 맞다. **둘 중 하나를 우연히 고르지 마라.**
...
```text nolines
s[::-1]                  61 ~ 64 us
"".join(reversed(s))   1090 ~ 1140 us
```
(Python 3.14 / Linux 기준 실측. 절대값은 기기마다 다르지만 자릿수 차이는 어디서나 같다.)

`[::-1]` 은 C 레벨에서 버퍼를 통째로 뒤집는다. `reversed` 는 **글자 하나마다 파이썬 객체를 만들어** `join` 에 넘긴다. 약 18배 차이다. 읽기 좋으라고 `join(reversed(...))` 를 쓸 이유가 없다 — `[::-1]` 이 더 짧기도 하다.
:::
...
문자열 100,000개를 잇는 세 가지, 여러 번 반복 측정.

```text nolines
"".join(PARTS)                  0.53 ~ 0.58 ms      이미 리스트다
"".join([p for p in PARTS])     1.55 ~ 1.68 ms      리스트 컴프리헨션
"".join(p for p in PARTS)       2.70 ~ 2.92 ms      제너레이터 표현식
```
(Python 3.14 / Linux 기준 실측. 절대값은 기기마다 다르지만 자릿수 차이는 어디서나 같다.)
...
길이 100,000 문자열에서 10종의 문자를 제거, 여러 번 반복 측정.

```text nolines
s.translate(table)                     0.29 ~ 0.30 ms
replace 10번 체인                       0.71 ~ 0.74 ms
"".join(c for c in s if c not in set)  4.2  ~ 4.6  ms
```
(Python 3.14 / Linux 기준 실측. 절대값은 기기마다 다르지만 자릿수 차이는 어디서나 같다.)

세 결과가 같은 문자열인지 확인하고 잰 값이다. `join`+제너레이터는 글자마다 파이썬 루프를 도니 자릿수가 다르다.

지울 문자 종류를 바꿔 가며 `replace` 체인과 `translate` 의 비를 재면 이렇다. 3종 1.4배, 10종 2.5배, 21종 3.3배. **`replace` 는 문자 종류만큼 원본을 다시 훑고, `translate` 는 몇 종이든 한 번만 훑는다.** 구두점 제거처럼 지울 문자가 고정돼 있으면 변환표를 모듈 상수로 한 번만 만들어라.
:::
...
```text nolines
s = s[:i] + "b" + s[i+1:]   반복       10.6 ~ 12.8 ms
list -> 수정 -> "".join                 0.28 ~ 0.44 ms
```
(Python 3.14 / Linux 기준 실측. 절대값은 기기마다 다르지만 자릿수 차이는 어디서나 같다.)

슬라이스 재조립은 수정 한 번마다 $O(n)$ 을 쓰므로 전체가 $O(n^2)$ 이 된다. **문자 단위 수정이 두 번 이상 나오면 `list(s)` 로 바꿔라.** 자료구조별 실제 비용은 [7.2](#/py-ds-cost)에 있다.
:::
...
- `str.isX()` 는 빈 문자열에서 전부 `False`, `all(...)` 은 `True` 다. 어느 쪽이 요구사항인지 정하고 골라라.
...
- 뒤집기는 `s[::-1]`(`join(reversed(s))` 보다 약 18배 빠르다). 여러 글자 치환·삭제는 `translate`. 문자 단위 수정이 반복되면 `list(s)`.
