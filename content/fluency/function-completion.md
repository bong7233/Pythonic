...
| `in the range [i - k, i + k]` | `range(i - k, i + k + 1)` — **`+1` 을 잊는 자리** |
...
::: danger 파이썬이 감춰 주는 타입 불일치 다섯 가지
| 실제로 반환한 것 | 기대한 것 | 왜 로컬에서 안 걸리는가 |
| --- | --- | --- |
| `5.0` | `5` | `5.0 == 5` 이 `True` |
| `True` | `1` | `bool` 은 `int` 의 하위 클래스 |
| `(0, 3)` | `[0, 3]` | 눈으로는 구별되지만 `assert` 를 튜플로 쓰면 통과 |
| `"True"` | `"true"` | `str(True)` 는 대문자로 시작한다 |
| `None` | `[]` | 답이 없는 케이스를 안 돌려 봤다 |
...
`assert got == want` 는 위의 다섯 가지를 전부 놓친다. 그래서 비교 함수를 하나 만들어 두고 문제마다 재사용한다. **이게 이 절에서 가져갈 도구 하나다.**
...
`local_peaks` 브루트포스에 `k = n` 을 준 실행 시간이다. 입력은 **모두 서로 다른 값**이고, "난수"는 `random.sample(range(10**9), n)`, "증가"는 `list(range(n))` 이다.

| n | 난수 | 증가 |
| --- | --- | --- |
| 1,000 | 0.89 ~ 0.93 ms | 32.7 ~ 36.1 ms |
| 2,000 | 1.81 ~ 2.39 ms | 135 ~ 143 ms |
| 4,000 | 3.78 ~ 3.85 ms | 543 ~ 575 ms |
| 8,000 | 7.99 ~ 8.24 ms | 2,179 ~ 2,400 ms |
| 16,000 | 측정 안 함 | 8,730 ~ 9,070 ms |
| 100,000 | 139.9 ~ 143.8 ms | 측정 중단 |

(Python 3.14.0rc2 / Linux 기준 실측. `timeit.repeat(number=1, repeat=5)` 의 최솟값과 최댓값이다. 마지막 줄만 `repeat=3`. 절대값은 기기마다 다르지만 자릿수 차이는 어디서나 같다.)

**난수 열은 n이 2배가 될 때 2배**, **증가 열은 n이 2배가 될 때 4배**가 된다. 같은 코드가 데이터 모양에 따라 $O(n)$ 처럼도 $O(n^2)$ 처럼도 보인다.

제한은 $10^5$ 이다. 난수 $10^5$ 개로는 브루트포스가 **140밀리초 남짓으로 통과한다.** 그런데 증가 열은 16,000 개에서 이미 9초다. **난수로만 스모크 테스트하면 통과하고, 채점 데이터가 정렬돼 있으면 시간 초과다.** 최대 크기 테스트에 정렬·역순 데이터를 반드시 섞어라([8.3](#/tle)).
:::

같은 문제를 창의 최댓값으로 푸는 구현은 `k` 와 무관하게 선형이다. 창 최댓값 자체는 [7.3](#/two-pointers)과 [11.2](#/stdlib-arsenal)의 `deque` 항목에 있으니 여기서는 결과만 쓴다 — `n = k = 100,000` 에서 난수·증가·감소 세 모양 모두 **33 ~ 43 ms** 로 데이터 모양에 흔들리지 않는다(같은 조건 실측).
...
- 파이썬은 타입 불일치를 감춘다. `True == 1`, `5.0 == 5`, `str(True) == 'True'`. **`assert got == want` 로는 못 잡는다** — `type(got) is type(want)` 까지 보는 비교 함수를 하나 만들어 두고 재사용해라.
...
- 제출 전 루틴은 넷이다. **예시(타입까지) → 엣지 배터리 → 무작위 대조 → 최대 크기 스모크.** 그리고 최대 크기 테스트는 **데이터 모양까지 바꿔야** 한다 — 같은 브루트포스가 난수 $10^5$ 개는 140밀리초, 증가 16,000개는 9초였다(실측). 난수로만 재면 통과해 버린다. 시간이 없으면 최대 크기부터.
...
A pair `(i, j)` with `i < j` is *fair* if `waits[i] + waits[j]` is at most `limit`
and `j - i` is at least 2.

Return the number of fair pairs. If there are none, return 0.
...
`count_fair(waits, limit) -> int` 를 완성해라. 먼저 **`at most` 와 `at least` 를 각각 어떤 부등호로 옮겼는지 주석으로 적고** 코드를 써라. 다음 세 케이스가 정답을 갈라 놓는다. 왜 그런지도 적어라.

```python
count_fair([1, 2, 3, 4], 5)      # 2
count_fair([0, 0, 0], 0)         # 1
count_fair([10 ** 9], 10 ** 9)   # 0
```

**2. 반환 타입 함정을 직접 만들어라 (10분)**

`is_balanced(entries: list[int]) -> bool` — `entries` 의 합이 0이면 `True`, 아니면 `False`. 단순하다. 그런데 **아래 네 가지 잘못된 구현을 모두 짜고**, 각각을 `strict_eq(got, True)` 로 걸어 보고 어떤 메시지가 나오는지 기록해라.
...
네 개 중 **`assert got == True` 를 통과해 버리는 것은 (b) 하나**다. 그리고 **(c) 는 `strict_eq` 조차 통과한다.** 둘 다 실제로 돌려서 확인하고, (c) 가 왜 통과하는지 한 문장으로 적어라.
