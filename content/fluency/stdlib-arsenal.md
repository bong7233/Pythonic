...
속도 걱정은 접어도 된다. 문자열 20만 개를 세는 데 `Counter(words)` 가 10.7~11.0 ms, `d[w] = d.get(w, 0) + 1` 수동 루프가 14.7~15.2 ms 였다(Python 3.14 / Linux 기준 실측. 절대값은 기기마다 다르지만 자릿수 차이는 어디서나 같다). 다만 **`Counter` 를 쓰는 진짜 이유는 속도가 아니라 코드가 짧아지는 것**이다.
...
**K가 작으면 힙이 10배 이상 빠르고, K가 커지면 오히려 3배 느려진다.** `nlargest` 는 $O(n \log K)$ 라 $K$ 가 $n$ 에 가까워지면 정렬보다 손해다. **"상위 몇 개"의 몇이 상수면 힙, 비율이면 정렬**로 기억해라.
:::
...
정렬된 20만 개 배열에 "x 이하가 몇 개?"를 2,000번 물었을 때, 그리고 2만 개를 하나씩 넣으며 정렬 상태를 유지할 때:
...
"질의가 여러 번 들어온다"는 문장을 보면 정렬 비용을 한 번 치르고 `bisect` 로 가는 것이 거의 항상 이긴다([8.3 시간 초과를 피하는 관용구](#/tle)). `insort` 도 삽입 자체는 원소 이동 때문에 $O(n)$ 인데 30배 이상 빠른 이유는, 그 이동이 **C 레벨 메모리 복사 한 번**이기 때문이다. 파이썬에서는 복잡도만큼이나 **상수가 어디서 도는가**가 중요하다([7.1 복잡도](#/complexity)).
:::
...
def analyze(log, recent_n=3):
    ...
    for line in log:
        ...
        (inbound if kind == "IN" else outbound)[sku] += int(qty)
...
본문 로직이 **12줄**이다. 도구 없이 짜면 딕셔너리 초기화와 조건문이 붙어 두 배가 되고, 두 배가 되면 그만큼 틀린다.
