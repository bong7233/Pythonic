...
| --- | --- |
| `print` 없음 | 7.2 ~ 8.0 ms |
| 루프마다 `print` | 299 ~ 339 ms |

(Python 3.14.0rc2 / Linux 기준 실측. `timeit.repeat(number=1, repeat=5)` 의 최솟값과 최댓값. 절대값은 기기마다 다르지만 자릿수 차이는 어디서나 같다.)

**최소 37배 느려졌다.** 그리고 채점기는 출력을 파일이나 파이프로 받으니 `/dev/null` 보다 더 느릴 수 있다. 알고리즘을 고치려고 30분을 쓴 다음에야 남겨 둔 `print` 가 원인이었다는 것을 발견하는 일이 실제로 일어난다. 출력이 많은 문제에서 `print` 자체가 얼마나 비싼가는 [8.2](#/io-optimize)에 있다.
:::
...
문제는 **$O(n)$ 검사를 루프 안에 넣는 것**이다. 위 `aggregate` 의 합계 검사를 루프 밖이 아니라 안에 두면 이렇게 된다. 데이터는 `(이름, 연체일)` 튜플 n개이고 이름은 전부 서로 다르다.

| n | 루프 안 `assert` | 없음 |
| --- | --- | --- |
| 2,500 | 27.2 ~ 28.6 ms | 0.25 ~ 0.27 ms |
| 5,000 | 105 ~ 107 ms | 0.56 ~ 0.71 ms |
| 10,000 | 422 ~ 430 ms | 1.29 ~ 1.41 ms |
| 20,000 | 1,673 ~ 1,734 ms | 2.68 ~ 3.05 ms |
...
def mine(rows):                               # 검증 대상. fee 는 고친 버전이다
    total = {}
    for name, days in rows:
        total[name] = total.get(name, 0) + fee(days)
...
리스트로 찾기
  n=  2500      68.8 ms
  n=  5000     262.6 ms   x3.8
  n= 10000    1049.6 ms   x4.0
  n= 20000    4606.3 ms   x4.4
딕셔너리로 찾기
  n=  2500       0.3 ms
  n=  5000       0.6 ms   x1.9
  n= 10000       1.3 ms   x2.2
  n= 20000       3.3 ms   x2.5
  n= 40000       8.6 ms   x2.6
  n= 80000      19.9 ms   x2.3
```

**배수만 보면 된다.** 4에 붙어 있으면 이차, 2에 붙어 있으면 선형에 가깝다. 리스트 버전은 $n$ 을 10배 늘리면 100배가 되고, 제한이 $2 \times 10^5$ 이면 절대 통과하지 못한다. 위 표에서 이미 $n = 20{,}000$ 에 4.6초다.

이 방법의 진짜 값어치는 **최대 크기까지 안 가도 판정된다**는 것이다. 20만 개로 돌려서 40분을 기다릴 필요가 없다. 2,500에서 20,000까지 네 번 재면 끝난다.

::: perf 배수를 어디까지 신뢰할 수 있는가
같은 측정을 `timeit` 최솟값으로 다시 잡은 것이다. 데이터는 `(이름, 연체일)` 튜플 n개, 이름은 `m000000` 형식으로 후보 n개에서 무작위 선택(그래서 서로 다른 이름은 n의 63% 남짓 — n=20,000 에서 12,675개), 연체일은 0~40 난수다.

| n | 리스트로 찾기 | 배수 | 딕셔너리로 찾기 | 배수 |
| --- | --- | --- | --- | --- |
| 2,500 | 64.7 ~ 69.1 ms | | 0.21 ~ 0.30 ms | |
| 5,000 | 255 ~ 259 ms | 3.9 | 0.44 ~ 0.57 ms | 2.1 |
| 10,000 | 1,023 ~ 1,069 ms | 4.0 | 0.93 ~ 1.28 ms | 2.1 |
| 20,000 | 4,148 ~ 4,823 ms | 4.1 | 2.45 ~ 3.23 ms | 2.6 |
...
| 관측 배수 | 판단 |
| --- | --- |
| 1.9 ~ 2.6 | 선형에 가깝다. 이 크기에서는 문제없다 |
| 3.5 ~ 4.5 | 이차. n을 10배 늘리면 100배 |
| 7 이상 | 삼차 이상이거나 지수 |

**$O(n)$ 과 $O(n \log n)$ 은 이 방법으로 구별되지 않는다.** 난수 float 리스트를 `sorted` 하는 시간은 크기를 두 배로 늘릴 때 2.40 ~ 2.43배가 되는데(같은 조건 실측), 위 딕셔너리 집계의 2.1 ~ 2.6과 겹친다. 그런데 시험장에서 그 구분은 필요 없다. **가려야 하는 것은 2 근처와 4 근처다**([7.1](#/complexity)).
:::

::: danger 데이터의 중복도가 기울기를 바꾼다
위 리스트 버전에서 이름이 서로 다를수록 `names` 가 길어지고 `in` 이 비싸진다. 그래서 **같은 코드가 데이터에 따라 기울기를 바꾼다.** 서로 다른 이름을 100개로 고정하면 이렇게 된다.

| n | 이름 후보 n개 | 이름 후보 100개 |
| --- | --- | --- |
| 2,500 | 64.2 ~ 70.1 ms | 4.6 ~ 4.8 ms |
| 5,000 | 251 ~ 267 ms | 9.3 ~ 9.5 ms |
| 10,000 | 1,030 ~ 1,174 ms | 18.9 ~ 19.5 ms |
| 20,000 | 4,293 ~ 4,428 ms | 37.9 ~ 39.3 ms |

(Python 3.14.0rc2 / Linux 기준 실측. 다른 조건은 위 표와 같다.)
...
- `assert` 로 단계 경계에 불변식을 박아라. **파싱 결과의 개수, 집계의 합계 보존, 선택의 최댓값.** 단 $O(n)$ 검사를 루프 안에 두면 $O(n)$ 이 $O(n^2)$ 이 된다 — 20,000개에서 2.7 ms 가 1.7초였다.
