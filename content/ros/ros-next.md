...
여기서 ROS 2 파트를 마친다. 1부에서 "변수는 상자가 아니라 이름표"라는 문장으로 시작해서, 그 이름표 하나(참조 카운팅)가 결국 이 파트 끝에서 `shared_ptr`이라는 다른 언어의 문법으로 다시 나타나는 것을 봤다. 동시성 파트에서 배운 GIL은 왜 rclpy 제어 루프가 지터에 취약한지를 설명하는 근거가 됐고, 성능 파트에서 배운 프로파일링은 "느낄 것 같다"가 아니라 "측정된 병목"으로 판단하라는 원칙으로 이 절에 다시 나왔다. 알고리즘과 코딩테스트 파트에서 훈련한 복잡도 감각은 로봇 노드 하나하나의 설계에도 그대로 적용된다. 언어는 도구다. 이 책이 가르치려 한 것은 특정 문법이 아니라, **왜 그렇게 동작하는지 묻는 습관**이었다.

여기까지가 "파이썬으로 무엇을 만들 수 있는가"에 대한 답이다. 그런데 아는 것과 **빈 화면 앞에서 시간 제한을 두고 짜내는 것**은 다른 능력이다. 남은 두 파트가 그 간극을 다룬다.

**다음 절**: [11.1 왜 손이 안 움직이는가](#/fluency-gap) — 알고리즘을 알아도 손이 안 움직이는 진짜 이유.
